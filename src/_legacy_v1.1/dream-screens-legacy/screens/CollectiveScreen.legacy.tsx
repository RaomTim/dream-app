...
export default function CollectiveScreen({ onClose }: CollectiveScreenProps) {
  ...
  return (
    ...
    >
      {/* Dream V1.2 — matter linen + demi-cercle aurore haut (Anima Mundi voûte) */}
      <Surface
        matter="linen"
        motion={true}
        style={{ position: 'absolute', inset: 0, zIndex: 0, pointerEvents: 'none', opacity: 0.85 }}
      />
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, opacity: 0.5, pointerEvents: 'none', zIndex: 0 }}>
      ...
      <div style={{ textAlign: 'center', padding: '0 20px 8px' }}>
        <SerifHeading size={24}>
          {fr ? 'Oracle de l\'Inconscient Collectif' : 'Collective Unconscious Oracle'}
        </SerifHeading>
        <div style={{
          fontStyle: 'italic', fontSize: 13, color: 'var(--fg-dim)',
          lineHeight: 1.5, marginTop: 8, maxWidth: 320, margin: '8px auto 0',
        }}>
          {fr
            ? 'Détecteur de réalité en formation. Les rêves coordonnent au niveau espèce.'
            : 'Reality-in-formation detector. Dreams coordinate at species level.'}
        </div>
      </div>
