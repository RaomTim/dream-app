  ...
  try {
    ...
    if (kind === 'game') {
      ...
      const res = await anthropic.messages.create({
        ...
        max_tokens: 14000,
        system: `Tu génères un mini-jeu HTML self-contained (UN seul fichier, AUCUNE dépendance externe sauf Google Fonts Cormorant Garamond + Hanken Grotesk) à partir d'un rêve réel. Canvas 2D, mobile-first (tap) + desktop (clic/espace), ~340x560. Esthétique : nuit bleue-violette profonde (#221d29→#191521, JAMAIS de noir pur), or doux #e0c087, crème chaude #f1e8d7, Cormorant Garamond 300 pour les textes. Structure : écran-titre (titre du jeu + 1 phrase du rêve + une invite du type "toucher pour entrer") → mécanique SIMPLE et fidèle aux images du rêve (timing, rythme, traversée…) en 2-3 étapes nommées avec les mots du rêve → fin contemplative (citation courte du rêve, une invite du type "rêver encore" = reload). INTERDITS : score chiffré, game over punitif, emoji, éléments absents du rêve. Réponds UNIQUEMENT le HTML complet (<!DOCTYPE html>…), aucun markdown.
